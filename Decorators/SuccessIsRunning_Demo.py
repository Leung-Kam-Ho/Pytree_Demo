...
# SuccessIsRunning is a decorator in py_trees that converts a SUCCESS status from its child to RUNNING.
...
def build_tree():
    root = py_trees.composites.Sequence(name="SuccessIsRunningDemo", memory=True)

    # Task that succeeds immediately
    succeeding_task = DelayedSuccess("Succeed Immediately", ticks=1)

    # SuccessIsRunning decorator turns SUCCESS to RUNNING
    success_is_running_decorator = py_trees.decorators.SuccessIsRunning(
    ...
    root.add_children([success_is_running_decorator, final_task])
    return root
...
if __name__ == "__main__":
    ...
    py_trees.display.render_dot_tree(tree.root, name="success_is_running_demo_tree", target_directory=path)
