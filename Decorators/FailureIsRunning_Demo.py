...
# FailureIsRunning is a decorator in py_trees that converts a FAILURE status from its child to RUNNING.
...
def build_tree():
    root = py_trees.composites.Sequence(name="FailureIsRunningDemo", memory=True)

    # Task that fails after delay
    failing_task = DelayedFailure("Fail After 1 Tick", ticks=1)

    # FailureIsRunning decorator turns FAILURE to RUNNING
    failure_is_running_decorator = py_trees.decorators.FailureIsRunning(
        name="Failure to Running",
        child=failing_task
    )

    # Final task to show sequence continues (but won't since it's running)
    final_task = DelayedSuccess("Done", ticks=1)

    root.add_children([failure_is_running_decorator, final_task])
    return root
...
if __name__ == "__main__":
    ...
    py_trees.display.render_dot_tree(tree.root, name="failure_is_running_demo_tree", target_directory=path)
